...
def people_counter():
	...
	while True:
		...
		for (objectID, centroid) in objects.items():
			...
			# to determine direction
				...
				if not to.counted:
					...
					if (not REVERSE and (direction < 0 and centroid[0] < W // 2)) or (REVERSE and (direction > 0 and centroid[0] > W // 2)):
						totalUp += 1
					...
					elif (REVERSE and (direction < 0 and centroid[0] < W // 2)) or (not REVERSE and (direction > 0 and centroid[0] > W // 2)):
						totalDown += 1
						date_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
		...
		if config["Log"]:
			log_data(move_in, in_time, move_out, out_time, max(0, totalDown - totalUp))
